...
async function getSongs(folder) {
    currfolder=folder
    let a = await fetch(`https://versionmaruti.github.io/${folder}/`);
...
const PlayMusic = (track,pause=false)=>{
    currentSong.src = `https://versionmaruti.github.io/${currfolder}/` + track;
    if(!pause){
        currentSong.play()
...
async function displayAlbums(){
    let a = await fetch(`https://versionmaruti.github.io/songs/`);
    let response = await a.text();
    let div = document.createElement("div")
    div.innerHTML = response
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if(e.href.includes("/songs/")){//**/
             ...
             let a = await fetch(`https://versionmaruti.github.io/songs/${folder}/info.json`)
